 ...
 */
public class WzUpload extends PiUpload {
	...
	public int[] upload(MboRemote dbMbo) {//dbMbo是已经得到任务id的
		...
		try {
			...
			String itemsql="select line_id from xj_taskinterface where checktime is not null and is_syn is null and orgname_tx = 'HRCN000' group by line_id";//检查过但未同步
			...
			String detailsql="select mobjectstate_cd ,place_tx ,djtask_id ,idpos_id, datatype_cd,checkresult_tx,exceptiondesc_tx,"
						...
						+ " from xj_taskinterface where checktime is not null and orgname_tx = 'HRCN000' and is_syn is null and line_id  = ? ";
			...
			taskps = connection2.prepareStatement(queryTaskSql);
			 ...
			 //更新中间库项目表状态为已同步
			 String interFaceSql = "update xj_taskinterface set is_syn ='Y' where djtask_id =?";
			 ...
			 String updateTask = "update ST_PI_TASK set status =?,ABNORMAL_POINT =?, "
			          + "IN_PLACE_RATE =?,PI_RATE =? ,MISSED_RATE =?,REAL_POINT =?,MISSED_POINT =? ,IGNORE_POINT  =? ,REAL_AREA  =? where  st_pi_taskid =?";
