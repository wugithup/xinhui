...
/**
 * com.shuto.mam.webclient.beans.safe.safepunish.SelectDeptDataBean 华东大区 阜阳项目
 * 安全处罚审批选择抄送部门databean
 * 
 * @author shanbh shanbh@shuoto.cn
 * @date 2017年4月6日 下午5:31:14
...
public class SelectDeptDataBean extends DataBean {
	...
	public synchronized int execute() throws MXException, RemoteException {
		...
		if (selectDept.size() > 0) {
			// 得到抄送部门
			MboSetRemote carbonLineSet = mainMbo.getMboSet("carbonline");
			for (int x = 0; x < selectDept.size(); x++) {
				...
				// 设置父级编号
				carbonMbo.setValue("APPLYID", mainMbo.getString("APPLYID"));

				// 设置所属应用
				carbonMbo.setValue("app", "SAFEPUNISH");
	...
	protected MboSetRemote getMboSetRemote() throws MXException,
		...
		// 得到已经选择的部门
		MboSetRemote adddeptSet = mainMbo.getMboSet("CARBONLINE");
